...
def generate_video(
    ...
    profiles = {
        "high":   ["-crf", "18", "-preset", "medium", "-tune", "fastdecode"],
        "medium": ["-crf", "23", "-preset",   "fast", "-tune", "fastdecode"],
        "low":    ["-crf", "28", "-preset",   "fast", "-tune", "fastdecode"],
    }
    ...
    ffmpeg_cmd = [
        ...
        "-vcodec", "libx264",
        "-pix_fmt", "yuv420p",
        *profiles.get(quality, profiles["medium"]),
        "-movflags", "+faststart",
        str(output),
    ]
...
Exemples :
  ...
  %(prog)s -q medium -o logout.mp4    # qualité medium
    ...
    p.add_argument("--fps", type=int, default=30, help="images par seconde")
    p.add_argument(
        "-d", "--duration", type=float, default=0.8,
        help="durée de la disparition en secondes (défaut 0.8)",
    )
    p.add_argument(
    ...
    p.add_argument(
        "-q", "--quality", choices=["low", "medium", "high"], default="medium",
