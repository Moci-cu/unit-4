...
ShellRoot {
    ...
    readonly property string activeMonitor: Hyprland.focusedMonitor
    ...
    function applyWallpaper(idx, monitor) {
        ...
        if (monitor === "both") {
            Quickshell.execDetached(["awww", "img", file])
        } else {
            Quickshell.execDetached(["awww", "img", "--outputs", monitor, file])
        }
    }
